...
export function parsePassportV2RuntimeConfig(env: RuntimeEnv): PassportV2RuntimeConfig | null {
  ...
  if ((env.VITE_MIDNIGHT_NETWORK?.trim() || 'preview') !== 'preview') {
    throw new TypeError('Passport v2 runtime is restricted to the Midnight Preview network');
  }
  ...
  const registry: FrozenCredentialRegistryReference = {
    registryContractAddress: required(env, 'VITE_CICO_REGISTRY_ADDRESS'),
  ...
  const referenda = wire.map((candidate, index) => parseReferendum(candidate, index, registry));
